import type { SpringOpts } from './types';
...
export const panDuration = (distance: number): number => Math.min(950, Math.max(320, Math.abs(distance) * 0.2));

/** Where the board should be `elapsed` ms into a pan from `from` to `to`. */
export function panPosition(from: number, to: number, elapsed: number): number {
  const dist = to - from;
...
export function rubberBand(over: number, limit = 56, c = 0.55): number {
  const o = Math.abs(over);
  return Math.sign(over) * (1 - 1 / ((o * c) / limit + 1)) * limit;
}

const springTerms = ({ stiffness = 400, damping = 26, mass = 1 }: SpringOpts) => {
...
export function spring(t: number, opts: SpringOpts = {}): number {
...
export function springDuration(opts: SpringOpts = {}): number {
...
export function springEasing(opts: SpringOpts = {}, steps = 32): string {
...
export type Claim = 'sheet' | 'scroll' | null;

export function sheetClaims({ dx, dy, scrollTop, atTop, onHandle }: { dx: number; dy: number; scrollTop: number; atTop: boolean; onHandle: boolean }): Claim {
