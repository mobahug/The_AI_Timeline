...
import { LEAD_CARD_IDS } from './leads';
import type {
  Accent, Adjacent, Category, Chain, Edge, Era, Event, EventRaw, ForwardLedger, Graph, Horizon, HorizonLedger,
  LinkRaw, LoadRow, Parent, Roads, Hop, Source, SourceStrength, Standing, Step, Strand, Thread, ThreadRow, EraGroup
} from './types';
...
export const FIRST: number = meta.span.first;
export const NOW: number = meta.span.now;
export const LAST: number = meta.span.last;
export const THREADS: Thread[] = meta.threads;
export const ERAS: Era[] = meta.eras;
export const CATEGORIES: Category[] = meta.categories;

const catById: Record<string, Category> = Object.fromEntries(CATEGORIES.map((c) => [c.id, c]));

export const catLabel = (id: string): string => (catById[id] ? catById[id].label : id);
export const catHue = (id: string): number => (catById[id] ? catById[id].hue : 250);
export const threadOf = (id: string): string => (catById[id] ? catById[id].thread : THREADS[0].id);

const raise = (e: EventRaw): Event => ({ ...e, thread: threadOf(e.category), future: e.year > NOW });

export const events: Event[] = (rawEvents as EventRaw[]).map(raise);

export const links: LinkRaw[] = rawLinks as LinkRaw[];

/** Warm-to-cool accent for a category, faded as a projection gets further out. */
export const fade = (year: number): number => (year <= NOW ? 0 : Math.min(1, (year - NOW) / (LAST - NOW)));
export const accent: Accent = (cat, f = 0) =>
...
const ANCHORS: [number, number][] = [[FIRST, 0], [1950, 0.05], [1980, 0.11], [2000, 0.19], [2010, 0.29], [2015, 0.4],
  [2018, 0.48], [2020, 0.55], [2022, 0.62], [2023, 0.69], [2024, 0.755], [2025, 0.815],
  [NOW, 0.865], [2030, 0.915], [2040, 0.96], [LAST, 1]];

export const yearFraction = (year: number): number => {
...
export const yearAtFraction = (v: number): number => {
  for (let i = 1; i < ANCHORS.length; i++) {
    if (v <= ANCHORS[i][1]) {
      const [ay, af] = ANCHORS[i - 1];
      const [by, bf] = ANCHORS[i];
...
export interface GraphExtra { nodes?: EventRaw[]; edges?: LinkRaw[] }
...
export function buildGraph(extra?: GraphExtra): Graph {
  ...
  const added: Edge[] = (x.edges || []).map((l, i) => ({ ...l, id: 'x' + i }));
  const edges = [...canon, ...added].filter((l) => index[l.from] && index[l.to] && l.from !== l.to);

  const adjacency: Record<string, Adjacent[]> = {};
  edges.forEach((l) => {
...
export function buildChain(graph: Graph, id: string, via?: string | null): Chain {
  const steps: Step[] = [];
  const seen = [id];
  let cur = id;
  for (let i = 0; i < 10; i++) {
...
export const CONFIDENCE: string[] = ['Likely', 'Uncertain', 'Speculative'];

export const projections = events.filter((e) => e.future);

export const parentsOf = (graph: Graph, id: string): Adjacent[] => (graph.adjacency[id] || []).filter((a) => !a.out);
...
export function groupByEra(items: Event[]): EraGroup[] {
...
export function strandOf(graph: Graph, id: string): Strand {
  const self = graph.index[id];
  const parents: Parent[] = parentsOf(graph, id)
    .map((a) => ({ event: graph.index[a.id], claim: a.claim, note: a.note }))
    .sort((a, b) => a.event.year - b.event.year);

  const depth: Record<string, number> = { [id]: 0 };
  const queue = [id];
  let hops = 0;
  while (queue.length) {
  ...
  const countPaths = (node: string, guard: Set<string>): number => {
...
export function horizonLedger(graph: Graph, horizon: Horizon): HorizonLedger {
  ...
  const jumps = argued.map((s) => s.strand.jump).filter((j): j is number => j !== null).sort((a, b) => a - b);
  ...
  return {
    ...
    confidence: CONFIDENCE.map((c): [string, number] => [c, set.filter((e) => e.confidence === c).length]).filter((p) => p[1]),
...
export function forwardLedger(graph: Graph): ForwardLedger {
...
export function standingNow(graph: Graph): Standing[] {
  return graph.all
    .filter((e) => !e.future)
    .map((e) => ({
      event: e,
...
export function roadsTo(graph: Graph, id: string, limit = 6, maxDepth = 12): Roads {
  const roads: Hop[][] = [];
  let truncated = false;

  const walk = (node: string, hops: Hop[], seen: Set<string>) => {
...
export function threadLedger(graph: Graph): ThreadRow[] {
...
export function loadBearing(graph: Graph, top = 6): LoadRow[] {
  const reach = (id: string): Event[] => {
...
export const SOURCE_KINDS: string[] = ['primary', 'paper', 'article', 'video', 'podcast', 'interview', 'encyclopedia'];

export function sourcesOf(event: Event | null | undefined): Source[] {
  if (!event) return [];
  const listed = Array.isArray(event.sources) ? event.sources : [];
  if (listed.length) {
    return listed.map((s): Source => ({
      kind: s.kind || 'article',
...
export function sourceStrength(event: Event | null | undefined): SourceStrength {
