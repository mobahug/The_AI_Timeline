...
import { NOW, THREADS, catLabel } from './data.js';

export const FINDINGS = spine.findings;
...
export function findingFor(year) {
  let hit = FINDINGS[0];
  for (const f of FINDINGS) { if (f.from <= year) hit = f; else break; }
  return hit;
}

export const findingByNumber = (n) => FINDINGS.find((f) => f.n === Number(n)) || null;

/** The year range a finding covers, closed at the next finding's start. */
export function spanOf(finding, lastYear) {
 ...
 */
export function buildLine(graph) {
  const all = graph.all;
  ...
  const joints = rows.slice(0, -1).map((row, i) => {
    const next = rows[i + 1];
...
export function readingOf(row) {
  ...
  const strings = (k) => k + (k === 1 ? ' string' : ' strings');
