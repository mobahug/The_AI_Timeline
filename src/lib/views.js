...
export function segmentsFor(route) {
  ...
  for (const seg of v.path.split('/')) {
    ...
    const value = route[PARAM_FIELD[seg.slice(1)] || seg.slice(1)];
...
export function matchSegments(segments) {
  ...
  for (const v of ordered) {
    ...
    for (let i = 0; i < pattern.length; i++) {
      if (pattern[i][0] === ':') {
        ...
        try { fields[PARAM_FIELD[name] || name] = decodeURIComponent(segments[i]); } catch { ok = false; break; }
