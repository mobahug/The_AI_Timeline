...
const INK_STEPS: Record<number, number> = { 1: 1, 2: 0.84, 3: 0.7, 4: 0.58, 5: 0.5 };
export const ink = (step: number = 3): string => 'rgba(243,240,234,' + (INK_STEPS[step] || step) + ')';
/** Ink on paper (the board's cards), same idea, dark on light. */
export const paperInk = (a: number): string => 'rgba(23,22,26,' + a + ')';
...
/* ─── Motion ─────────────────────────────────────────────────────────────
   ...
   the edge of a bounce (motion.ts). A state change (colour, border, opacity) is
...
export type Tier = 'page' | 'section' | undefined;
export const micro = (opacity: number = 5, tier?: Tier): CSSProperties => ({
...
export type Tone = 'quiet' | 'loud' | 'dim';
export type Size = 'sm' | 'md' | 'lg' | 'hero';
const BUTTON_SIZE: Record<Size, CSSProperties> = {
...
export const button = (tone: Tone = 'quiet', size: Size = 'md'): CSSProperties => ({
...
export const tag = (cat: string, accent: Accent): CSSProperties => ({
...
const SHELL_WIDTH: Record<string, number> = { read: 920, route: 1100, wide: 1400 };
export const shell = (kind: string = 'wide'): CSSProperties => ({
...
export const yearBit: CSSProperties = {
...
const HEADLINE: Record<HeadlineSize, { year: string; title: string }> = {
...
export const headline = (size: HeadlineSize = 'panel'): { year: CSSProperties; title: CSSProperties } => ({
...
export const reading = (cat: string, accent: Accent, size: 'md' | 'lg' = 'md'): CSSProperties => ({
  margin: 0,
...
export const frame = (future: boolean = false): CSSProperties => ({
  borderRadius: 3,
...
export const sourceLink: CSSProperties = { ...micro(1), letterSpacing: '0.14em' };
