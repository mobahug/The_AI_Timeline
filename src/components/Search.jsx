...
import { ALL_ENTITIES } from '../lib/files.js';
import { INK, MONO, SANS, SERIF, ROW_RULE, ink, micro } from '../lib/styles.js';
import { Link, useNav } from './kit.jsx';
...
function score(q, primary, secondary) {
...
export function searchAll(graph, query, limit = 6) {
  ...
  const cards = graph.all
    ...
    .map(({ e }) => ({ kind: 'card', id: e.id, label: e.title, sub: e.year + ' · ' + catLabel(e.category), to: { view: 'card', id: e.id } }));
  const leads = LEADS
    .map((l) => ({ s: score(q, l.title, l.question + ' ' + l.blurb), l }))
    .filter((x) => x.s).sort((a, b) => b.s - a.s).slice(0, 3)
    .map(({ l }) => ({ kind: 'lead', id: l.id, label: l.title, sub: 'Lead · ' + l.rungs.length + ' rungs', to: { view: 'lead', id: l.id } }));
  const names = ALL_ENTITIES
    .map((x) => ({ s: score(q, [x.label, ...(x.aka || [])].join(' '), x.role || x.short || ''), x }))
    .filter((y) => y.s).sort((a, b) => b.s - a.s).slice(0, limit)
    .map(({ x }) => ({ kind: x.kind, id: x.id, label: x.label, sub: x.kind === 'term' ? 'Term · ' + (x.short || '') : (x.kind === 'person' ? 'Person · ' : 'Organisation · ') + (x.role || ''), to: { view: x.kind, id: x.id } }));
  return [
...
export default function SearchBox({ graph, value, onChange, narrow, style }) {
