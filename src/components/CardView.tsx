...
const KIND_LABEL: Record<string, string> = { primary: 'Primary', paper: 'Paper', article: 'Article', video: 'Video', podcast: 'Podcast', interview: 'Interview', encyclopedia: 'Encyclopaedia' };

/** The article lead with the sentence that bears on this entry highlighted. */
const Lead = ({ event, extract }: { event: Event; extract: string }) => {
...
const SourceBlock = ({ src, event, page }: { src: Source; event: Event; page: Shot }) => {
...
export interface CardViewProps {
  graph: Graph;
  /** The board's file drawer hands over only the id; a page hands over its whole route. */
  route: Pick<Route, 'id'> & Partial<Route>;
  media: Media;
  /** Inside the board's panel: no page column, no nav. */
  embedded?: boolean;
}

function CardView({ graph, route, media, embedded }: CardViewProps) {
