...
import { THREADS, TICKS, FIRST, NOW, CATEGORIES, accent, buildChain, catLabel, catHue, fade, yearAtFraction } from '../lib/data';
import { PAD, metrics, layout, stringPath } from '../lib/layout';
import { leadById, leadChain } from '../lib/leads';
import { useMode } from '../lib/mode';
import { MANILA, MONO, PAPER, RED, RED_LIT, CYAN, SERIF, DUR, EASE, FADE, STATE, micro, paperInk } from '../lib/styles';
import { Btn, Empty, RouteProvider } from './kit';
import { easeOut, panDuration, rubberBand, sheetClaims, springDuration, springEasing } from '../lib/motion';
import type { Claim } from '../lib/motion';
import type { Edge, Event, Graph, Media, Metrics, Node, Route, RoutePatch, Step } from '../lib/types';
...
const Card = React.memo(function Card({ node, m, img, lit, subject, raised, hoverable, onClick, onHover, onFocusCard }: CardProps) {
  const e = node.event;
  const f = fade(e.year);
  return (
    <button
...
/** A string as the board draws it: the edge, its colours, and the curve. */
interface BoardString extends Edge { cat: string; future: boolean; tone: string; lit: string; d: string; mx: number; my: number }
interface StringPathProps { s: BoardString; strong: boolean; onChain: boolean; dim: boolean }
const StringPath = React.memo(function StringPath({ s, strong, onChain, dim }: StringPathProps) {
...
const keyOf = (s: Step): string => (s.lead ? 'lead:' + s.lead + ':' + s.rung : s.from + '>' + s.to);
const patchOf = (s: Step): RoutePatch => (s.lead
  ? { lead: s.lead, rung: s.rung, clue: null, id: null }
  : { clue: { from: s.from, to: s.to }, id: null, lead: null, rung: null });

/** The sheet's three stops on a narrow screen. */
type Stop = 'peek' | 'half' | 'full';

/** A pull of the sheet, from the press to the release. */
interface SheetDrag {
  x0: number; y0: number; h0: number; h: number; moved: boolean; t: number; y: number; v: number; raf: number | null;
  onHandle: boolean; claimed: Claim; scrollTop: number; mouse: boolean; captured: boolean;
}
...
export interface BoardViewProps {
  ...
  route: Route;
  navigate: Navigate;
  onYear?: (year: string, fraction: number) => void;
  full: boolean;
}

export default function BoardView({ items, graph, media, route, navigate, onYear, full }: BoardViewProps) {
  ...
  const [viewport, setViewport] = useState<[number, number]>([FIRST, FIRST + 50]);
  ...
  const panTo = useCallback((left: number, top?: number) => {
  ...
  const keepVisible = useCallback((id: string) => {
    const el = scroller.current;
    const p = positions[id];
    if (!el || !p) return;
  ...
  const pendingPan = useRef<string | null>(null);
  ...
  const wantCentre = useRef<Step | null>(null);
  const showChain = useCallback((steps: Step[], index: number, replaceUrl: boolean) => {
    setPinned(null);
    setHover(null);
    setFileOpen(false);
    setChain(steps);
  ...
  const openLead = useCallback((id: string, rung?: number | null) => {
    const l = leadById[id];
    if (!l) return;
  ...
  const openClue = useCallback((from: string, to: string) => {
  ...
  const openChain = useCallback((id: string, atStep?: number | null) => {
  ...
  const restoredAt = useRef<string>('');
  useEffect(() => {
    const el = scroller.current;
  ...
  // of the frame.
  const headYear = useRef<number | null>(null);
  const wasFull = useRef<boolean>(full);
  useEffect(() => {
  ...
  const lastFilter = useRef<string>(filterKey);
  useEffect(() => {
  ...
  const onScroll = useCallback(() => {
    ...
    const fr = (x: number) => Math.min(1, Math.max(0, (x - PAD) / (board3.width - PAD * 2)));
  ...
  const clickCard = useCallback((event: Event) => {
  ...
  useEffect(() => {
    ...
    const h = panelRef.current.querySelector<HTMLElement>('h2, h3');
  ...
  const [sheet, setSheet] = useState<Stop>('half');
  ...
  const sheetAtTop = useRef<boolean>(false);
  sheetAtTop.current = sheet === 'full';
  const claim = (d: SheetDrag, dx: number, dy: number): Claim => {
  ...
  const onSheetDown = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const el = panelRef.current;
    if (!el) return;
    const onHandle = !!((e.target as HTMLElement).closest && (e.target as HTMLElement).closest('[data-sheet-handle]'));
    sheetDrag.current = {
  ...
  const onSheetUp = () => {
    ...
    const stops = (['peek', 'half', 'full'] as Stop[]).map((k): [Stop, number] => [k, stopH(k)]);
    let next: Stop | null | undefined;
  ...
  const onSheetSettled = (e: ReactTransitionEvent<HTMLDivElement>) => {
      ...
      <div ref={frameRef} style={{ position: 'relative', flex: 1, minHeight: 0, overflow: 'clip' }}>
        <div
          ...
          style={{
            ...
            border: '1px solid rgba(243,240,234,0.12)', borderRadius: 3, touchAction: m.fits ? 'pan-x' : 'auto',
