...
import type { Edge, Entity, EntityKind, Graph, Lead, Route, ViewId } from '../lib/types';
...
const KIND_LABEL: Record<EntityKind, string> = { person: 'Person of interest', org: 'Organisation', term: 'Term' };
const LIST: Record<EntityKind, Entity[]> = { person: PEOPLE, org: ORGS, term: TERMS };
const INDEX_VIEW: Record<EntityKind, ViewId> = { person: 'files', org: 'files', term: 'glossary' };
const INDEX_LABEL: Record<EntityKind, string> = { person: 'The files', org: 'The files', term: 'The glossary' };

export interface EntityViewProps { graph: Graph; route: Route }

function EntityView({ graph, route }: EntityViewProps) {
  const kind = route.view as EntityKind;
  const entity = entityById(kind, route.id as string);
  ...
  // The one-liner and the body, by kind: a person or an organisation has a
  // role and a bio; a term has a short line and a definition.
  const line = entity.kind === 'term' ? entity.short : entity.role;
  const body = entity.kind === 'term' ? entity.definition : entity.bio;
  ...
  return (
    <div style={shell('read')}>
      <div data-year={first ? first.year : undefined} style={{ padding: '30px 0 0' }}>
        ...
        {line && (
          <p style={{ margin: '14px 0 0', font: '400 clamp(14px,1.5vw,17px)/1.5 ' + SANS, color: ink(3), maxWidth: '56ch', textWrap: 'pretty' }}>{line}</p>
        )}
