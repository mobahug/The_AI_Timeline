...
import { VIEWS, VIEW_IDS, TABS, SITEMAP_VIEWS, RETIRED, tabOf, viewById } from '../src/lib/views';
import { hrefFor, parse, resolve } from '../src/lib/url';
import { metaFor } from '../src/lib/meta';
import { FIRST, NOW, LAST, buildGraph, yearFraction, yearAtFraction, TICKS, forwardLedger } from '../src/lib/data';
import { ink, micro, button, badge, shell, headline } from '../src/lib/styles';
import type { HeadlineSize, Size } from '../src/lib/styles';
import type { Route, ViewId } from '../src/lib/types';
...
describe('the view registry is the one list of pages', () => {
  ...
  it('files every child under a tab that exists', () => {
    VIEWS.filter((v) => v.parent).forEach((v) => expect(viewById[v.parent as ViewId].tab, v.id).toBe(true));
...
describe('routes round-trip through the URL', () => {
  ...
  it('reads every path it writes', () => {
    const routes: (Pick<Route, 'view'> & Partial<Route>)[] = [
    ...
    routes.forEach((r) => {
      ...
      Object.keys(r).forEach((k) => expect(back[k as keyof Route], k + ' of ' + url.href).toEqual(r[k as keyof Route]));
    });
  });
  ...
  it('starts a new view from a clean slate but carries the filter', () => {
    const prev = { view: 'board', id: 'alexnet', clue: null, finding: null, category: 'power', query: 'x' } as Route;
...
describe('page meta derives from the route', () => {
  it('names a card by its year and title', () => {
    const e = graph.all[0];
    expect(metaFor({ view: 'card', id: e.id } as Route, graph).title).toBe(e.year + ' · ' + e.title + ' · The AI Timeline');
  });

  it('spells out a clue as A claim B', () => {
    const l = graph.edges[0];
    const m = metaFor({ view: 'board', clue: { from: l.from, to: l.to } } as Route, graph);
  ...
  it('never types a year the data does not carry', () => {
    const m = metaFor({ view: 'landing' } as Route, graph);
  ...
  it('falls back to the registry for a titled view and to the site for the rest', () => {
    expect(metaFor({ view: 'archive' } as Route, graph).title).toBe('The archive · The AI Timeline');
    expect(metaFor({ view: 'landing' } as Route, graph).title).toContain('The AI Timeline');
  });
});
...
describe('the style helpers hold the contrast floor', () => {
  it('never writes an ink step below 0.5 on the ground', () => {
    [1, 2, 3, 4, 5].forEach((s) => {
      const a = Number((ink(s).match(/,([\d.]+)\)$/) as RegExpMatchArray)[1]);
  ...
  it('gives buttons three tones and four sizes, and no caller needs its own padding', () => {
    ...
    expect(button('quiet', 'nonsense' as unknown as Size).padding).toBe(button('quiet', 'md').padding);
  });
  ...
  it('has three page widths and three headline scales', () => {
    expect(shell('read').maxWidth as number).toBeLessThan(shell('route').maxWidth as number);
    expect(shell('route').maxWidth as number).toBeLessThan(shell('wide').maxWidth as number);
    (['page', 'panel', 'row'] as HeadlineSize[]).forEach((k) => expect(headline(k).title.font).toContain('Instrument Serif'));
  });
});
