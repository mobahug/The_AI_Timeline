...
import { PEOPLE, ORGS, TERMS, ALL_ENTITIES, names, entitiesOf, appearancesOf, entityById } from '../src/lib/files.js';
import { buildGraph, events } from '../src/lib/data.js';
...
describe('the files', () => {
  ...
  it('finds a name by tag and by text, whole words only, and never in a source', () => {
    const hinton = entityById('person', 'geoffrey-hinton');
    expect(names({ title: 'Hinton leaves Google', summary: '', why: '' }, hinton)).toBe(true);
    expect(names({ title: 'x', summary: 'Hintonism is not a word', why: '' }, hinton)).toBe(false);
    expect(names({ title: 'x', summary: '', why: '', people: ['geoffrey-hinton'] }, hinton)).toBe(true);
    expect(names({ title: 'x', summary: '', why: '', sources: [{ quote: 'Hinton said' }] }, hinton)).toBe(false);
    const lean = entityById('term', 'formal-verification');
    expect(names({ title: 'x', summary: 'checked in Lean', why: '' }, lean)).toBe(true);
    expect(names({ title: 'x', summary: 'a lean startup', why: '' }, lean)).toBe(false);
    const winter = entityById('term', 'ai-winter');
    expect(names({ title: 'x', summary: 'The first AI Winter', why: '' }, winter)).toBe(true);
  });
  ...
  it('leaves few names without a card, and gives the busiest their due', () => {
    ...
    expect(appearancesOf(graph, entityById('org', 'openai')).length).toBeGreaterThan(10);
    expect(appearancesOf(graph, entityById('person', 'geoffrey-hinton')).length).toBeGreaterThan(4);
  });
