...
import { easeOut, panDuration, panPosition, rubberBand, sheetClaims, spring, springDuration, springEasing } from '../src/lib/motion.js';
...
describe('the sheet gives past its ends, and springs back', () => {
  ...
  it('a flick carries: released at speed it overshoots more than released still', () => {
    const peakAt = (v0) => { let p = 0; for (let t = 0; t <= 1; t += 0.004) p = Math.max(p, spring(t, { v0 })); return p; };
...
describe('a touch on the sheet goes to the sheet or to the scroll, the native way', () => {
  const at = (o) => sheetClaims({ dx: 0, dy: 0, scrollTop: 0, atTop: false, onHandle: false, ...o });
