...
import { LEADS, LEAD_CARD_IDS, buildLead, leadChain, leadsOf } from '../src/lib/leads.js';
...
describe('leads.json', () => {
  it('has unique kebab-case ids and every field a page needs', () => {
    ...
    LEADS.forEach((l) => {
      ...
      ['title', 'question', 'blurb', 'standing'].forEach((k) => expect(typeof l[k], l.id + '.' + k).toBe('string'));
  ...
  it('ends every lead on the record or a scenario, never past the board', () => {
    LEADS.forEach((l) => {
      ...
      expect(b.span.from).toBeLessThanOrEqual(b.span.to);
