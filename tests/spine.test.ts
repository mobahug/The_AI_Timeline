...
import { FINDINGS, findingFor, spanOf, buildLine, readingOf } from '../src/lib/spine';
...
describe('the spine is well formed', () => {
  ...
  it('every lead names a real entry', () => {
    FINDINGS.filter((f) => f.lead).forEach((f) => {
      expect(graph.index[f.lead as string], f.n + ' ' + f.title + ' leads with ' + f.lead).toBeTruthy();
    });
  });
  ...
  it('every lead falls inside its own span', () => {
    FINDINGS.filter((f) => f.lead).forEach((f) => {
      ...
      const year = graph.index[f.lead as string].year;
  ...
  it('every lead is a card the board already treats as significant', () => {
    FINDINGS.filter((f) => f.lead).forEach((f) => {
      const e = graph.index[f.lead as string];
      const linked = (graph.adjacency[f.lead as string] || []).length > 0;
